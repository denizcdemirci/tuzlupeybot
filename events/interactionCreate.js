...
module.exports = (client, inter) => {
  if (inter.type === InteractionType.ApplicationCommand) {
    ...
    if (command.permissions && !inter.member.permissions.has(command.permissions)) return inter.reply({
      embeds: [
        new EmbedBuilder()
          .setColor('#ff0000')
    ...
    if (DJ.enabled && DJ.commands.includes(command) && !inter.member._roles.includes(inter.guild.roles.cache.find(x => x.name === DJ.roleName).id)) return inter.reply({
      embeds: [
        new EmbedBuilder()
          .setColor('#ff0000')
          .setDescription(`❌ | This command is reserved For members with \`${DJ.roleName}\` `)
      ],
      ephemeral: true,
    });
